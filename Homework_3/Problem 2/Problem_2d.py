...
for i in range(iterates - 1):
    ...
    p_avg = np.dot(probabilities, susceptibilities) / np.sum(susceptibilities)
    average_prob.append(p_avg)
...
for n in range(lines): 
    lower_s = values_s[:,n]
    plt.plot(time_points, lower_s, label=f"Group {n+1} Sahana", color=colors[n])
    plt.grid(True)

plt.title('Susceptible population --Sahana')
plt.xlabel('Time')
plt.ylabel('Proportion of Susceptible Individuals')
plt.legend()
plt.show()

plt.figure()
plt.plot(time_points[:-1], average_prob, label="Average Susceptibility --Sahana", color='black')
plt.xlabel("Time")
plt.ylabel("Average Susceptibility")
plt.title("Average Susceptibility vs. Time -- Sahana")
plt.show()
